...
class miniClassifier():
    ...
    def conv_layer(self, inFeed, tfFilter, bias,padding_type):
    ...
    def build(self):
        ...
        self.conv1_1 = self.conv_layer(self.input,weights["w1_1"],biases["b1_1"],'VALID')
        self.pool1 = self.avg_pooling(self.conv1_1,"avg_pool1")

        self.conv2_1 = self.conv_layer(self.pool1, weights["w2_1"], biases["b2_1"],'VALID')
        self.pool2 = self.avg_pooling(self.conv2_1,"avg_pool2")

        self.conv3_1 = self.conv_layer(self.pool2, weights["w3_1"], biases["b3_1"],'VALID')        
        ...
        self.sig1 = tf.nn.sigmoid(self.fc2)
    ...
    def compile(self,iters = 150,batches = None):        
        for it in range(iters):
            if(batches == None):
                batch_x = self.Img
                batch_y = self.Label
                self.run_compute(batch_x,batch_y)                
            else:
                for batch_pointer in range(len(self.Img)//batches):
                    ...
                    self.run_compute(batch_x,batch_y)
            if(it % 20 == 0):
                    self.learn_rate = self.learn_rate / (1+ 0.01*it)
